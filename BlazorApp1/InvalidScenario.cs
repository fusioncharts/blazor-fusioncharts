...
namespace Microsoft.FusionChartsInterop.Tests
{
    public class ChartsServiceTest
    {
        ...
        [Fact]
        public async Task CallFusionChartsFunction_GetJSONData_InvalidArguments()
        {
            // Arrange
            var mockJsRuntime = new MockJSRuntime();
            var fusionChartsService = new FusionChartsService(mockJsRuntime);
            var functionName = "getJSONData";
            var chartId = "ChartId";
            var invalidArgs = new object[] { "InvalidArgument" }; // Invalid arguments

            // Act & Assert
            await Assert.ThrowsAsync<ArgumentException>(async () =>
            {
                await fusionChartsService.CallFusionChartsFunction(functionName, chartId, invalidArgs);
            });
            Assert.Empty(mockJsRuntime.Invocations);
        }

        [Fact]
        public async Task CallFusionChartsFunction_SetJSONData_InvalidArguments()
        {
            // Arrange
            var mockJsRuntime = new MockJSRuntime();
            var fusionChartsService = new FusionChartsService(mockJsRuntime);
            ...
            var chartId = "ChartId";
            var invalidJsonData = 123; // Invalid JSON data, not a string
            var args = new object[] { invalidJsonData };
