...
namespace Microsoft.FusionChartsInterop.Tests
{
    ...
    public class FusionChartsServiceTests
    {
        ...
        [Fact]
        public async Task AddAnnotations_CallsInvokeVoidAsyncWithCorrectArguments()
        {
            // Arrange
            var mockJsRuntime = new MockJSRuntime();
            var fusionChartsService = new FusionChartsService(mockJsRuntime);
            ...
            var chartId = "ChartId";
            var args = Array.Empty<object>(); // Replace with actual arguments

            // Act
            await fusionChartsService.addAnnotations(functionName, chartId, args);

            // Assert
            Assert.Single(mockJsRuntime.Invocations);
            Assert.Equal("FusionCharts.addAnnotations", mockJsRuntime.Invocations[0].Item1);
            Assert.Equal(new object[] { functionName, chartId, args }, mockJsRuntime.Invocations[0].Item2);
        }
    }
}
